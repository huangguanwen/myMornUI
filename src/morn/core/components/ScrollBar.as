...
package morn.core.components {
	...
	public class ScrollBar extends Component {
		...
		public function set target(value:InteractiveObject):void {
			...
			if (_target) {
				...
				if(Multitouch.supportsTouchEvents){
				    ...
				    _target.removeEventListener(TouchEvent.TOUCH_BEGIN,onTargetTouchBegin)
				}else{
			...
			if (value) {
				...
				if (_touchScrollEnable) {
					if(Multitouch.supportsTouchEvents){
						_target.addEventListener(TouchEvent.TOUCH_BEGIN,onTargetTouchBegin)
					}else{
		 ...
		 * 
		 */		
		protected function onTargetTouchBegin(e:TouchEvent):void{
			App.timer.clearTimer(tweenMove);
			if (!this.contains(e.target as DisplayObject)) {
				_target.addEventListener(TouchEvent.TOUCH_END,onStageTouchEnd2)
				_target.addEventListener(TouchEvent.TOUCH_MOVE,onStageTouchMove)
		 ...
		 * 
		 */		
		protected function onStageTouchMove(e:TouchEvent):void{
		
			_lastOffset = _slider.direction == VERTICAL ?e.stageY - _lastPoint.y : e.stageX - _lastPoint.x;
		 ...
		 * 
		 */		
		protected function onStageTouchEnd2(e:TouchEvent):void{
			_target.removeEventListener(TouchEvent.TOUCH_END,onStageTouchEnd2)
			_target.removeEventListener(TouchEvent.TOUCH_MOVE,onStageTouchMove)
