package com.book.yao.components.flipbook 
{
	...
	{
		private var _pageArray:Vector.<String>=new Vector.<String>();//page数组
		private var _currentPageIndex:uint = 0;//当前页序号
		...
		private var _nextPageloader:Loader = new Loader();
		private var _prevPageloader:Loader = new Loader();
		private var _currentPageLoader:Loader = new Loader();
		
		private var _nextPageContainer:Sprite = new Sprite();
		private var _prevPageContainer:Sprite = new Sprite();
		private var _currentPageContainer:Sprite = new Sprite();
		
		//因为在本地测试的时候loader.close不能停止，所以设置一下三个属性标记loader状态
		private var _currentLoaderClosed:Boolean = true;
		private var _nextLoaderClosed:Boolean = true;
		private var _prevLoaderClosed:Boolean = true;
		
		private var pageWidth:Number=1920
		private var pageHeight:Number=1080
		...
		public function addPage(link:String):void{
				_pageArray.push(link);
				_pageCount=_pageArray.length
				if(_pageArray.length==3){
					loadCover()
				}
		...
		private function currentLoadComHandler(evn:Event):void
		{
			trace(_isJump,_targetPageType)
			var loader_info:LoaderInfo=evn.target as LoaderInfo
			var content:DisplayObject=loader_info.content
			
			var bitm:BitmapData=new BitmapData(pageWidth,pageHeight)
			var matrix:Matrix=new Matrix()
			matrix.scale(pageWidth/content.width,pageHeight/content.height)
			bitm.draw(content,matrix,null,null,null,true);
			...
			if (!_currentLoaderClosed)
			{
				switch(_targetPageType)
				{
					case "jump_next":
					{
						...
						_nextPageContainer.addChild(bitmap);
					...
					case "jump_prev":
					{					
	//	trace(_prevPageContainer.numChildren,	_prevPageContainer,visible)
						if (_prevPageContainer.numChildren != 0)
						{						
							_prevPageContainer.removeChildAt(0);
						}
						
						//_currentPageLoader.content.width=pageWidth
						//_currentPageLoader.content.height=pageHeight
						_prevPageContainer.addChild(bitmap);
					...
					case "cover":
					{
						//_currentPageLoader.content.width=pageWidth
						//_currentPageLoader.content.height=pageHeight
						_currentPageContainer.addChild(bitmap);
		...
		private function nextLoadComHandler(evn:Event):void
		{
		...
		private function prevLoadComHandler(evn:Event):void
		{
		...
		{
		//	trace(index,_isJump,_taretPageIndex,_currentPageIndex)
			_isJump = true;
		//	trace(index,_isJump,_taretPageIndex,_currentPageIndex)
			...
			//trace(_targetPageType)
			
			
			
			_currentPageLoader.load(new URLRequest(_pageArray[index]));
		...
		public function loadCover():void
		{
			...
			_taretPageIndex = 0;
			_currentPageLoader.load(new URLRequest(_pageArray[0]));
			_currentLoaderClosed = false;
		}
		...
		public function loadPrev():void
		{
			if (!isStart)
			{
				_prevPageloader.load(new URLRequest(_pageArray[_currentPageIndex - 1]));
		...
		public function flipCompleteHandler(type:String):void
		{
			switch(type)
			{
				case FlipType.PREV:
					...
					var curr:DisplayObject=_prevPageContainer.getChildAt(0)
					curr.width=pageWidth
					curr.height=pageHeight
				    
					_currentPageContainer.addChild(curr);
				...
				case FlipType.NEXT:
					...
					var curr2:DisplayObject=_nextPageContainer.getChildAt(0)
				    curr2.width=pageWidth
				    curr2.height=pageHeight
					_currentPageContainer.addChild(curr2);
		...
		//page数组
		public function set pageArray(array:Vector.<String>):void
		{
			_pageArray = array;
			_pageCount = _pageArray.length;
		}
		 ...
		 * 
		 */		
		public function loadPage(page:String):void{
			if(_pageArray.indexOf(page)==-1){
				_pageArray.push(page)
				_pageCount = _pageArray.length;
			}
		...
		public function get pageArray():Vector.<String>
		{
			return _pageArray;
		}
